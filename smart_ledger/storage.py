...
class Storage:
    ...
    def _init_db(self, *, seed_categories: bool = True):
        ...
        self._migrate_add_column(cur, "stock_holdings", "position_entry_price", "REAL NOT NULL DEFAULT 0")
    ...
    def update_stock_holding_full(self, holding: StockHolding) -> bool:
        ...
        cur.execute(
            """UPDATE stock_holdings SET name = ?, buy_price = ?, current_price = ?,
               ...
               cost_compensation = ?, trades_synced_at = ?, realized_pnl = ?,
               position_entry_price = ? WHERE id = ?""",
            (holding.name, holding.buy_price, holding.current_price,
             ...
             holding.trades_synced_at, holding.realized_pnl, holding.position_entry_price, holding.id),
        )
        self.conn.commit()
        return cur.rowcount > 0
